...
public class SoldierEnemyBehaviour : EnemyBaseActionClass, ICanHoldItems {

	private bool enemyBehaviourActive = false;
	...
	protected override void InitializeEnemy() {
		...
		Debug.Log ("Soldier enemy id is " + GetCombatantID ());
	}

	//CUSTOM SOLDIER BEHAVIOUR
	public void EnableEnemyBehaviour() {
		if (enemyBehaviourActive == false) {
	...
	protected override IEnumerator EnemyControl() {
		//Continuously
		while (true) {
			//Make sure that the enemy behaviour is active, instead of the typical system.  
			if (enemyBehaviourActive) {
				...
				//When the behaviour is not active.  
				yield return null;
			}
		}
	}
