...
public class RaycastAttackUtilities : MonoBehaviour {
	...
	public static CharacterHealthPanelManager LookForEnemyViaLinecast (Vector2 origin, float distToEnemyOffset, float yOffsetToEnemy, float enemyWithinAreaBound, int characterFacingDirection, string attackingCharacterGUID) {
			...
			for (int i = 0; i < linecastResult.Length; i++) {
				...
				if (linecastResult[i].collider.transform.parent != null && linecastResult[i].collider.transform.parent.parent != null) {
					...
					if (healthPanelTransform.GetComponent <CharacterHealthPanelManager> () != null &&
					    healthPanelTransform.GetComponent <CharacterBaseActionClass> ().GetCombatantID ().Equals (attackingCharacterGUID) == false) {
	...
	public static T LookForEnemyViaLinecast <T> (Vector2 origin, float distToEnemyOffset, float yOffsetToEnemy, float enemyWithinAreaBound, int characterFacingDirection, string attackingCharacterGUID) {
		//Pretty much all of this is calculation for the eventual linecast.  
		Vector2 enemyWithinAreaVectorBound = new Vector3 (enemyWithinAreaBound, 0);
		Vector2 distToEnemyVectorLength = new Vector3 (distToEnemyOffset, 0);
		Vector2 yOffsetVector = new Vector3 (0, yOffsetToEnemy);
